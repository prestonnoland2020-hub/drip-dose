...
export function settingFor(g, um) {
  const P = g.points
...
export function micronsFor(g, setting) {
...
export function fmtSetting(g, s) {
...
export function stepSetting(g, s, n) { return clampStep(g, Number(s) + n * (Number(g.step) || 1)) }
export const grinderName = g => g ? `${g.brand} ${g.model}` : null
