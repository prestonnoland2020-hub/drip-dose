...
export async function render() {
  ...
  mount(`${top('')}
    <div style="text-align:center;margin:40px 0 26px"><h1>Sign in to POR</h1><p class="muted">Scan a bag, get the recipe, brew it, rate it. Your coffees and brews, on every device. Free.</p></div>
  ...
  $('g').onclick = async () => { try { const { error } = await signInGoogle(); if (error) toast('Google sign-in is not available — use the email link') } catch { toast('No connection — try again in a moment') } }
  $('e').onclick = async () => { const email = $('email').value.trim(); if (!email) return $('email').focus(); $('e').disabled = true; let error = null; try { ({ error } = await signInEmail(email)) } catch { error = { message: 'No connection — try again in a moment' } } const m = $('msg'); m.hidden = false; m.textContent = error ? error.message : `Link sent to ${email}. Open it on this device.`; $('e').disabled = false }
}
