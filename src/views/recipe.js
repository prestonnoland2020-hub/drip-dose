...
async function load() {
  ...
  el.innerHTML = `
    ...
    ${r.grind_setting
      ? `<div class="card accent" style="margin-top:12px"><div class="row between"><span class="eyebrow">Grind</span>${confidenceTag(rec.confidence)}</div><div style="display:flex;align-items:center;gap:16px;margin-top:6px"><div class="num" style="font-size:52px;font-weight:700;line-height:1;letter-spacing:-.03em">${esc(r.grind_setting)}</div><div style="font-size:15px">on your <b>${esc(r.grinder.brand)} ${esc(r.grinder.model)}</b>${r.grinder.dial ? `<div class="small muted">${esc(r.grinder.dial)}</div>` : ''}</div></div>${r.grinder.verified === false ? `<div class="small" style="margin-top:8px">First time POR has seen this grinder — the number is an estimate until you rate a cup.</div>` : ''}</div>`
      : `<div class="row between" style="margin:12px 0 4px"><span class="small muted">Grind: <b>${esc(r.grind_hint || r.grind_label)}</b>${r.grind_microns ? ` · ${r.grind_microns} µm` : ''}${uid() ? ` · <a href="#/setup" style="color:var(--accent)">add your grinder for a setting</a>` : ''}</span>${confidenceTag(rec.confidence)}</div>`}
    <div class="section"><div class="eyebrow"><b>Recipe</b></div>${stepsList(r.steps)}${byId(r.method).basis ? `<p class="small muted" style="margin-top:8px">${esc(byId(r.method).basis)}</p>` : ''}</div>
    ...
    <div class="section"><h3>Why this recipe?</h3><div class="why">${esc(r.grind_setting ? rec.why.replace('the grind sits on the fine side of the range', `your ${r.grinder.brand} sits a step finer than its usual pour-over setting`).replace('the grind coarser to keep bitterness in check', `your ${r.grinder.brand} a step coarser to keep bitterness in check`) : rec.why)}</div></div>
