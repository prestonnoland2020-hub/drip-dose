...
export async function render({ params, name }) {
  ...
  mount(`${top('', { back: own ? null : '#/home', right: own ? `<a class="iconbtn" href="#/settings" aria-label="Settings">${icon(I.edit)}</a>` : '' })}
    ...
    <div class="small muted" style="margin:12px 0 10px">${counts.followers} follower${counts.followers === 1 ? '' : 's'} · ${counts.following} following</div>
    <div class="chips" style="margin:0 0 14px">${own ? `<a class="chip" href="#/setup">My setup</a><a class="chip" href="#/settings">Preferences</a><button class="chip" id="out">Sign out</button>` : `<button class="chip" id="follow" aria-pressed="${following}">${following ? 'Following' : 'Follow'}</button>`}</div>
