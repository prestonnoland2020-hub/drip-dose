...
export async function render({ params, name }) {
  ...
  mount(`${top('', { back: own ? null : '#/home', right: own ? `<a class="iconbtn" href="#/settings" aria-label="Settings">${icon(I.edit)}</a>` : '' })}
    ...
    <div class="row" style="margin:14px 0;gap:8px">${own ? `<a class="chip" href="#/setup">My setup</a><a class="chip" href="#/settings">Preferences</a><button class="chip" id="out">Sign out</button>` : `<button class="chip" id="follow" aria-pressed="${following}">${following ? 'Following' : 'Follow'}</button>`}<span class="small muted" style="margin-left:auto">${counts.followers} followers · ${counts.following} following</span></div>
