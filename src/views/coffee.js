...
export async function render({ params }) {
  ...
  mount(`${top('', { back: '#/discover' })}
    ...
    <div class="row" style="margin-top:10px;gap:8px"><button class="chip" id="follow" aria-pressed="${following}">${following ? 'Following' : 'Follow coffee'}</button>${c.roaster && c.roaster !== 'Unknown roaster' ? `<a class="chip" href="#/roaster/by/${encodeURIComponent(c.roaster)}">More from ${esc(c.roaster)}</a>` : ''}${c.product_url ? `<a class="chip" href="${esc(c.product_url)}" target="_blank" rel="noopener">Buy ↗</a>` : ''}</div>
