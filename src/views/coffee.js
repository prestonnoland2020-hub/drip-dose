...
export async function render({ params }) {
  ...
  mount(`${top('', { back: '#/discover' })}
    ...
    <div class="row" style="margin-top:10px;gap:8px"><button class="chip" id="follow" aria-pressed="${following}">${following ? 'Following' : 'Follow coffee'}</button>${c.roaster && c.roaster !== 'Unknown roaster' ? `<a class="chip" href="#/discover?roaster=${encodeURIComponent(c.roaster)}">More from ${esc(c.roaster)}</a>` : ''}</div>
