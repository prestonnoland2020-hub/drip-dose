...
export async function render() {
  ...
  mount(`${top('')}
    ...
    <div class="section"><div class="section-h"><h2>From the community</h2><a href="#/discover?tab=brews">See all</a></div><div id="feed" class="list"><div class="skeleton"></div></div></div>`)

  if (c) { loadRec(c); loadLast(c) }
  loadFeed()
...
function heroCard(c) {
  return `<div class="card"><a href="#/coffee/${c.id}" style="display:block"><div class="coffee-hero">${bagImg(c)}<div>${coffeeTitle(c)}${coffeeMeta(c)}${c.tasting_notes ? `<div class="taste small">${esc(c.tasting_notes)}</div>` : ''}</div></div></a><div id="recbox" style="margin-top:14px"><div class="skeleton"></div></div></div>`
}
...
async function loadRec(c) {
  ...
  try {
    ...
    set({ rec })
    const r = rec.recipe
    el.innerHTML = `<div class="row between" style="margin-bottom:8px"><span class="small"><b>${esc(r.method_name)}</b> · ${esc(String(r.dose).replace(/\.0$/, ''))} g → ${Math.round(r.water)} g · ${r.temp_note ? esc(r.temp_note) : tempStr(r.temp, state.units)}${r.grind_setting ? ` · grind ${esc(r.grind_setting)}` : ''}</span><a href="#/brew" class="small" style="color:var(--accent);white-space:nowrap">Change</a></div>
      <a class="btn primary big" href="#/recipe">${icon(I.play)} Start brew</a>`
  } catch (e) {
    el.innerHTML = `<a class="btn primary big" href="#/brew">Choose a method</a>`
  }
}
...
async function loadFeed() {
  const rows = (await social.feed(6).catch(() => [])).slice(0, 3)
  const el = $('feed')
